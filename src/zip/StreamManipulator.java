...
class StreamManipulator {
	...
	/**
	 * Get the next n bits but don't increase input pointer. n must be less or equal
	 * 16 and if you if this call succeeds, you must drop at least n-8 bits in the
	 * next call.
	 * 
	 ...
	 * bigger or equal n before, to make sure that enough bits are in the bit
	 * buffer.
	 */
	 ...
	 * output[offset]. You have to make sure, that the buffer is byte aligned. If
	 * not enough bytes are available, copies fewer bytes.
	 * 
	...
	public void setInput(byte[] buf, int off, int len) {
		...
		/*
		 * We want to throw an ArrayIndexOutOfBoundsException early. The check is very
